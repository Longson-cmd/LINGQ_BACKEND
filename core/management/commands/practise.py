...
def create_lesson_obj(course, text_file_path,  idx_lesson):
    ...
    text_ext = os.path.splitext(text_file_path)[1]
    ...
    with open(text_file_path, 'rb') as f:
        lesson_obj.text_file.save(f'text_{idx_lesson}{text_ext}', File(f), save = True)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):    
        ...
        if Courses.objects.filter(user = first_user).exists():
            ...
            return
        list_file_names = ['text0.txt', 'text1.txt', 'text2.txt', 'text3.txt', 'check.txt']
...
#     def handle(self, *args, **options):
#         try:
#             first_user = User.objects.get(username = 'exmaple@.com')
#         except:
#             first_user= User.objects.create_user(
